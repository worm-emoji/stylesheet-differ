...
test('supports css objects', () => {
  const response = dif({ body: { color: 'red' } }, { body: { color: 'blue' } })
...
test('supports media query', () => {
  const response = dif(
    `@media (min-width: 750px) { body { background-color: white; } }`,
    `@media (min-width: 750px) { body { background-color: black; } }`
  )
  expect(response).toBe(
